...
-- Loop and coroutine setup
coroutine.resume(coroutine.create(function()
    while getgenv().MemorySpoofer.Enabled and RS.Heartbeat:Wait() do
        Script.Functions.ChangeText()
    end
end))

coroutine.resume(coroutine.create(function()
    while getgenv().MemorySpoofer.Enabled do
        task.wait(getgenv().MemorySpoofer.Delay)
        -- Generate random memory values without Texts
        local randomValue1 = math.random(getgenv().MemorySpoofer.Lowest, getgenv().MemorySpoofer.Maximum)
        local randomValue2 = math.random(10, 99)
        local randomValue3 = math.random(getgenv().MemorySpoofer.Lowest, getgenv().MemorySpoofer.Maximum)
        local randomValue4 = math.random(10, 99)
    end
end))
...
coroutine.resume(coroutine.create(function()
    while getgenv().MemorySpoofer.Enabled do
        task.wait(5)
        -- Generate random memory values
        local randomValue5 = math.random(getgenv().MemorySpoofer.Lowest, getgenv().MemorySpoofer.Maximum)
        local randomValue6 = math.random(100, 999)
        local randomValue7 = math.random(100, 999)
    end
end))
